...
function load()
{
    ...
    {
        var map2 = new GMap2(document.getElementById("map2"), {draggableCursor: "crosshair"}, {draggingCursor: "move"}, {mapTypes: [G_NORMAL_MAP, G_SATELLITE_MAP, G_HYBRID_MAP]});
        map2.addMapType(G_PHYSICAL_MAP);

        // definit le centre et le niveau de zoom de départ
        map2.setCenter(new GLatLng(48.579823784230655, 7.7635520696640015), 12);
        map2.addControl(new GMenuMapTypeControl());
        map2.addControl(new GLargeMapControl());
        map2.addControl(new DragZoomControl());
        map2.addControl(new GOverviewMapControl());
        map2.addControl(new GScaleControl());
        map2.enableScrollWheelZoom();
        GEvent.addListener(map2, "click", function (overlay, latlng)
        {
            if (overlay) {
                return;
            }
            latitude = Math.abs(latlng.lat())
            if (latlng.lat() < 0)
            {
                directionlat = "S"
            }
            ...
            degrelat = parseInt(latitude)
            minutelat = parseInt(60 * (latitude - degrelat))
            secondeintlat = parseInt(60 * (60 * (latitude - degrelat) - parseInt(60 * (latitude - degrelat))))
            secondedeclat = parseInt(100 * (60 * (60 * (latitude - degrelat) - parseInt(60 * (latitude - degrelat))) - parseInt(60 * (60 * (latitude - degrelat) - parseInt(60 * (latitude - degrelat))))))
            longitude = Math.abs(latlng.lng())
            if (latlng.lng() < 0)
            {
                directionlng = "W"
            }
            ...
            degrelng = parseInt(longitude)
            minutelng = parseInt(60 * (longitude - degrelng))
            secondeintlng = parseInt(60 * (60 * (longitude - degrelng) - parseInt(60 * (longitude - degrelng))))
            secondedeclng = parseInt(100 * (60 * (60 * (longitude - degrelng) - parseInt(60 * (longitude - degrelng))) - parseInt(60 * (60 * (longitude - degrelng) - parseInt(60 * (longitude - degrelng))))))
            var myHtml = "<b>Coordonnées (GPS) :</b><br/>" + "X = " + latlng.lng() + "<br>Y = " + latlng.lat();

            // donner X et Y
            document.getElementById("user_coordx").value =
                    document.getElementById("user_coordx").defaultValue = latlng.lng();
            document.getElementById("user_coordy").value =
                    document.getElementById("user_coordy").defaultValue = latlng.lat();

            map2.openInfoWindow(latlng, myHtml);
            niveauZoom = map2.getZoom();
            if (niveauZoom < 12)
            {
                map2.setCenter(new GLatLng(latlng.lat(), latlng.lng()), 12);
            }
            else
            {
                map2.setCenter(new GLatLng(latlng.lat(), latlng.lng()), niveauZoom);
            }
            centreCarte = map2.getCenter()
            document.getElementById("AfficheCentreCarte").innerHTML = "&nbsp;&nbsp;&nbsp;" + centreCarte;
...
function DragZoomControl(opts_boxStyle, opts_other, opts_callbacks) {
    ...
    this.globals.options = {
        ...
        buttonZoomingHTML: 'Zoomez une zone sur la carte',
...
DragZoomControl.prototype.initialize = function (map) {
    ...
    G.mapCover.onselectstart = function () {
        return false
    };
...
DragZoomControl.prototype.getDefaultPosition = function () {
    return new GControlPosition(G_ANCHOR_TOP_LEFT, new GSize(3, 290));
};
